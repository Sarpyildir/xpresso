...
import { fetchNoteById } from "../utils/fetchNote.js";
  ...
  useEffect(() => {
    fetchNoteById(noteId)
      .then((res) => {
        ...
        setEditableDescription(note.description);
        setFormattedDate(formatDate(note.created_at));

        // Fetch user data after note data is successfully fetched and set
        return fetchUserById(note.postedBy);
      })
      .then((userData) => {
        setUser(userData.data);
      })
      .catch((error) => {
        console.error("Failed to fetch note or user:", error);
      });
  }, [noteId]); // Depend on noteId
